...
def get_input():
    ...
    return [{"min": int(item.split(" ")[0].split("-")[0]),
             "max": int(item.split(" ")[0].split("-")[1]),
             "letter":item.split(" ")[1][0],
             "password":item.split(" ")[2],
             "raw_input":item} for item in parsed_items]
...
def get_solution_1():
    ...
    for password_data in passwords_data:
        #letter_occurences = len((password_data['password']+' ').split(password_data['letter']))
        letter_occurences = password_data['password'].count(password_data['letter'])

        if password_data['min'] <= letter_occurences <= password_data['max']:
            valid_cnt += 1
        else:
            #print("corrupted pass: %s" % password_data['raw_input'])
            #print(" \\ %s letter_occurences: %s" % (password_data['letter'], letter_occurences))
            corrupted_cnt += 1
    print("valid passwords: %s" % valid_cnt)
...
def get_solution_2():
    ...
    for pass_data in passwords_data:
        ...
        first_pos = pass_data['min'] - 1
        second_pos = pass_data['max'] - 1
        if pass_data['password'][first_pos] == pass_data['letter']:
            correct_occurences += 1
        if pass_data['password'][second_pos] == pass_data['letter']:
            correct_occurences += 1

        if correct_occurences != 1:
            pass
            #print("corrupted pass: %s" % pass_data['raw_input'])
        else:
            valid_cnt += 1
            #print("------------------------ OK pass: %s" % pass_data['raw_input'])
