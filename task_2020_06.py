...
def get_input():
    ...
    for line in input_data.splitlines():
        if line == "":
            empty_lines_cnt += 1
            if len(entry_data.strip()) > 0:
                merged_entries.append(entry_data.strip())
            entry_data = ""
            continue
        entry_data += line
...
def get_input_2(test=False):
    ...
    for line in effective_input:
        if line == "":
            empty_lines_cnt += 1
            if len(entry_data) > 0:
                merged_entries.append(entry_data)
            entry_data = []
            continue
        entry_data.append(line.strip())
...
def get_solution_1():
    declaration_data_by_group = get_input()
    #print(str(declaration_data_by_group))
    unique_yeses_per_group = []
    for group_answers in declaration_data_by_group:
        unique_yeses_per_group.append(len(list(set(list(group_answers)))))
    print("sum of unique yeses per group: %s" % sum(unique_yeses_per_group))


def get_solution_2():
    declaration_data_by_group = get_input_2(test=False)
    #print(str(declaration_data_by_group))
    common_yesses_for_all_groups = []
    group_index = 0
    for answers_within_group in declaration_data_by_group:
        common_yesses_for_group = []
        #print("group %s" % group_index)
        first_answer = answers_within_group[0]
        other_answers = answers_within_group[1:]
        for answer in list(first_answer):
            common_answer = True
            #print("testing answer: %s" % answer)
            for other_answer in other_answers:
                if answer not in other_answer:
                    common_answer = False
            if common_answer:
                common_yesses_for_group.append(answer)
        common_yesses_for_all_groups.append(common_yesses_for_group)

        group_index += 1
    #print("common_yesses_for_all_groups: %s", common_yesses_for_all_groups)
    print("sum of common answers: %s" % sum([len(item) for item in common_yesses_for_all_groups]))
