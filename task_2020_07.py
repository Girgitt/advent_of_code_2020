...
input_data = """bright gray bags contain 2 bright gold bags, 5 dull lavender bags.
...
def get_input(test=False):
    ...
    empty_lines_cnt = 0
    if test:
        effective_input = test_data.splitlines()
    else:
        effective_input = input_data.splitlines()
    for line in effective_input:
        if "no other bags" in line:
            continue
        outer_bag = " ".join(line.split(" ")[:2])
        inner_bags = []
        if "," in line:
            inner_bag_parts = line.split(",")
            for inner_bag_token in inner_bag_parts:
                inner_bag_name = " ".join(inner_bag_token.split(" ")[-3: -1])
                inner_bag_count = inner_bag_token.split(" ")
                try:
                    inner_bag_count = inner_bag_count[len(inner_bag_count)-4]
                except IndexError:
                    print(inner_bag_count)
                    continue
                inner_bags.append(inner_bag_count+":"+inner_bag_name)
        else:
            inner_bag_name = " ".join(line.split(" ")[-3: -1])
            inner_bag_count = line.split(" ")
            try:
                inner_bag_count = inner_bag_count[len(inner_bag_count) - 4]
            except IndexError:
                print(inner_bag_count)
                continue
            inner_bags.append(inner_bag_count+":"+inner_bag_name)
        merged_entries[outer_bag] = inner_bags

        #inner_bags = " ".join(line.split(" ")[2:])

    return merged_entries


def get_solution_1():
    bags_tree = get_input(test=False)
    #print("bags_tree: %s" % str(bags_tree))
    bag_levels = []
    prev_level = []
    iter_cnt = 0
    iter_max = 100
    ...
    while iter_cnt < iter_max:
        ...
        for bag in bags_tree:
            if bag in added_bags:
                continue
            if "shiny gold" in [item.split(":")[-1] for item in bags_tree[bag]]:
                current_level.append(bag)
                added_bags.append(bag)
            else:
                for bag_from_prev_level in prev_level:
                    if bag_from_prev_level in [item.split(":")[-1] for item in bags_tree[bag]]:
                        if bag in added_bags:
                            continue
                        current_level.append(bag)
                        added_bags.append(bag)
        ...
        #print("current_level level: %s" % current_level)
    if iter_max - iter_cnt < 2:
        print("REACHED ITERATION LIMIT")
    print("added_bags: %s" % added_bags)
    print("result: %s" % len(added_bags))
...
def get_solution_2():
    ...
    while iter_cnt < iter_max:
        iter_cnt += 1
        next_level = []
        for bag_to_check in current_level:
            try:
                multiplier = int(bag_to_check.split(":")[0])
                bag = bag_to_check.split(":")[1]
            except ValueError:
                multiplier = 1
                bag = bag_to_check

            #print("multi %s x %s" % (multiplier, bag))
            for i in range(multiplier):
                added_bags.append(bag)
            try:
                for internal_bag in bags_tree[bag]:
                    #print("multi %s x %s" % (multiplier, internal_bag))
                    for i in range(multiplier):
                        next_level.append(internal_bag)
            except KeyError:
                pass
        if len(next_level) == 0:
            break
    ...
    if iter_max - iter_cnt < 2:
        print("REACHED ITERATION LIMIT")
    #print("added_bags: %s" % added_bags)
    print("result: %s" % (len(added_bags)-1))